...
void create_mpi_params(struct info_param *param, struct MPI_info_param *mpi_param) {
	int i;
	MPI_Aint address[N_PARAMS], base_address;

	// sizes and types
    for (i = 0; i < N_PARAMS; i++) {
        mpi_param->sizes[i] = 1;
        mpi_param->types[i] = (i < 4) ? MPI_INT : MPI_FLOAT;
    }

	// distances
	mpi_param->distances[0] = 0;
	MPI_Get_address(&(param->nconf), &address[0]);
	MPI_Get_address(&(param->nchip), &address[1]);
	MPI_Get_address(&(param->max_iter), &address[2]);
	MPI_Get_address(&(param->scale), &address[3]);
	MPI_Get_address(&(param->t_ext), &address[4]);
	MPI_Get_address(&(param->tmax_chip), &address[5]);
	MPI_Get_address(&(param->t_delta), &address[6]);
	for (i = 1; i < N_PARAMS; i++) mpi_param->distances[i] = address[i] - address[0];

	// crear tipo
	MPI_Type_create_struct(N_PARAMS, mpi_param->sizes, mpi_param->distances, mpi_param->types, &(mpi_param->param));
	MPI_Type_commit(&(mpi_param->param));
}